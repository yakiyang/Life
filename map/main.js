...
function maleBinomialZ(d) {
    ...
    //assume population mean of males is 0.5
...
function refreshGenderPlot() {
    ...
    var axisY = d3.svg.axis().scale(sy).orient('left');

    canvas.select('#x_axis').attr('transform', 'translate(0,' + height / 2 + ')').call(axisX);
    canvas.select('#y_axis').attr('transform', 'translate(40,0)').call(axisY);
...
function refreshPopMap(){
    ...
    var prmin = d3.min(G.targetCensusData, popRatio);
    var prmax = d3.max(G.targetCensusData, popRatio);
    ...
    towns.transition().attr('fill', function (d) { return ratio2color(popRatio(d)); });
...
function createAxes() {
    ...
    var sy = d3.scale.linear().domain([-3, 3]).range([0, 600]);
    var canvas = d3.select('#gp-canvas');
...
function prepareTargetRegion(selected) {
    ...
    /************** bar chart ******************/
    var bcEntries = d3.select('#bar-chart-proper')
                      .selectAll('.bc-entry')
                      .data(
                        G.targetCensusData,
                        function (d) {
...
function init(error, data) {
    ...
    var regionSelection = d3.select('#region-selection')
                            .selectAll('option')
    ...
    d3.select('#pm-rsvg-wrapper')
      ...
      .call(pmzoom)
      .append('g')
      .attr('id', 'pm-zoom-or-zoomless')
      .append('g')
      .attr('id', 'pm-canvas');


    /***************** start default target city/country ****************/
